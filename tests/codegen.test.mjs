...
for(const text of ["qc.h(0)","qc.cx(0, 1)","qc.s(1)","qc.rz(-45 * pi / 180, 2)","qc.ry(-90 * pi / 180, 1)","qc.rz(-225 * pi / 180, 2)","qc.measure_all()","StatevectorSampler","counts_qiskit =", "bits[::-1]", "print(counts_abc)"])assert.ok(code.includes(text),text);
