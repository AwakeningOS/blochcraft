...
export function initCircuit({onSelect,onState,onCircuit}){
  ...
  function renderTools(){
    toolBox.innerHTML="";
    for(const tool of tools){const button=document.createElement("button");button.textContent=tool.label;button.className=selected===tool.id?"active":"";button.onclick=()=>{selected=tool.id;renderTools();if(tool.type!=="delete")onSelect?.(tool.sync||tool.id)};toolBox.append(button)}
    ...
    if(number&&slider){
      const update=(value)=>{selectedAngle=Math.max(-360,Math.min(360,Number(value)||0));number.value=selectedAngle;slider.value=selectedAngle;for(const button of meaning.querySelectorAll("[data-angle]"))button.classList.toggle("active",+button.dataset.angle===selectedAngle)};
