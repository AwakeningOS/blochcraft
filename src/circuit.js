...
const bitMarkup=(bits)=>[...bits].map((bit,q)=>`<span class="qubit-${QUBITS[q].toLowerCase()}">${bit}</span>`).join("");
...
export function initCircuit({onSelect,onState,onCircuit}){
  ...
  function selectColumn(c,q=0){selectedColumn=c;step.value=c+1;render();syncSelection(c,q,100)}
  function renderGrid(){grid.innerHTML="";grid.style.setProperty("--circuit-columns",columns.length);const corner=document.createElement("div");corner.className="column-corner";grid.append(corner);for(let c=0;c<columns.length;c++){const number=document.createElement("button");number.type="button";number.className="column-number"+(selectedColumn===c?" selected-column":"");number.textContent=c+1;number.title=`${c+1}列目を選択`;number.setAttribute("aria-label",`${c+1}列目を選択`);number.onclick=()=>selectColumn(c);grid.append(number)}for(let q=0;q<3;q++){const label=document.createElement("div");label.className=`wire-label qubit-${QUBITS[q].toLowerCase()}`;label.textContent=QUBITS[q];grid.append(label);for(let c=0;c<columns.length;c++){const cell=document.createElement("button");cell.className="circuit-cell"+(selectedColumn===c?" selected-column":"");const text=document.createElement("span");text.className="gate-label";text.textContent=cellText(columns[c],q);cell.append(text);cell.onclick=()=>cellText(columns[c],q)&&selected!=="delete"?selectColumn(c,q):place(q,c);grid.append(cell)}}}
  function place(q,c){const tool=tools.find(item=>item.id===selected);selectedColumn=c;step.value=c+1;if(tool.type==="delete"){columns[c].multi=null;columns[c].singles[q]=null}else if(tool.type==="controlled"){columns[c]=blank();columns[c].multi={...tool}}else{columns[c].multi=null;columns[c].singles[q]={...tool,...(tool.type==="rotation"?{angle:selectedAngle}:{})}}render();syncSelection(c,q)}
  function syncSelection(c,q,progress=0){const column=columns[c],singleQ=column.singles[q]?q:column.singles.findIndex(Boolean);let operation=column.multi||column.singles[singleQ];const card=document.getElementById("circuit-selection");if(!operation){card.textContent=`${c+1}列目：操作なし`;return}card.innerHTML=`<strong>${c+1}列目：${operationLabel(operation)}</strong><span>球体上の意味：${operation.sphere}</span>`;onSelect?.({operation,target:operation.target??singleQ,column:c,startState:simulate(columns,c),source:"circuit",progress})}
  ...
  function renderState(){const steps=+step.value,state=simulate(columns,steps);stepOut.textContent=steps===columns.length?"最後まで":steps===0?"入力前":`${steps}列目の後`;document.getElementById("state-table").innerHTML=`<div class="state-table-header"><span class="bit-order" aria-label="ビット順 A B C"><b class="qubit-a">A</b><b class="qubit-b">B</b><b class="qubit-c">C</b></span></div>`+state.map((amplitude,index)=>{const probability=abs2(amplitude)*100,bits=index.toString(2).padStart(3,"0");return`<div class="state-row"><code class="bit-string" aria-label="A ${bits[0]}、B ${bits[1]}、C ${bits[2]}">${bitMarkup(bits)}</code><span><i style="width:${probability}%"></i></span><b>${probability.toFixed(1)}%</b><small class="amplitude">振幅 ${formatAmplitude(amplitude)}</small></div>`}).join("");document.getElementById("qubit-summary").innerHTML=QUBITS.map((name,q)=>{const s=reducedSummary(state,q),mixed=s.length<.999;return`<div><strong class="qubit-${name.toLowerCase()}">${name}</strong><span>北 |0⟩ ${(s.p0*100).toFixed(1)}% / 南 |1⟩ ${(s.p1*100).toFixed(1)}%</span><small>${mixed?`関係あり・単体矢印 ${(s.length*100).toFixed(1)}%`:`単体球で表現可能・(X,Y,Z)=(${s.x.toFixed(2)}, ${s.y.toFixed(2)}, ${s.z.toFixed(2)})`}</small></div>`}).join("");onState?.(state);onCircuit?.(columns.map(column=>({singles:[...column.singles],multi:column.multi})),steps)}
  function syncViewerToStep(){
    renderState();
    const steps=+step.value;
    if(steps===0){
      const first=columns.findIndex(columnUsed);
      if(first<0)return;
      selectedColumn=first;
      renderGrid();
      syncSelection(first,0,0);
      return;
    }
    let column=Math.min(steps,columns.length)-1;
    while(column>=0&&!columnUsed(columns[column]))column--;
    if(column<0)return;
    selectedColumn=column;
    renderGrid();
    syncSelection(column,0,100);
  }
  function render(){renderTools();updateColumnControls();renderGrid();renderState()}step.oninput=syncViewerToStep;step.value=columns.length;render();return{simulate:()=>simulate(columns,+step.value)};
}
