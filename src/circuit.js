...
export function initCircuit({onSelect,onState,onCircuit}){
  ...
  function insertColumn(after){if(columns.length>=MAX_COLUMNS)return;const index=selectedColumn<0?columns.length:selectedColumn+(after?1:0);columns.splice(index,0,blank());selectedColumn=index;moveStepToEnd();render()}
  ...
  document.getElementById("column-add-one").onclick=()=>addColumns(1);document.getElementById("column-add-four").onclick=()=>addColumns(4);document.getElementById("column-insert-before").onclick=()=>insertColumn(true);document.getElementById("column-insert-after").onclick=()=>insertColumn(false);document.getElementById("column-delete").onclick=deleteColumn;
